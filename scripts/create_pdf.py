...
def get_templates(ext):
    available_files = (fn for fn in iglob(f"../{ext}_templates/*.{ext}") if purify(fn) != "common")
    ret = dict()
...
for song, song_name in tqdm(songs.items()):
    ...
    if song_type in ["m", "p"]:
        ...
        for stanza_sheet in stanza_sheets:
            ...
            fn = song + "-" + str(stanza_sheet)
            if args.skip_bad and (fn in bad):
                continue

            current_template = ly_template_contents[ly_templates[song_type]]

            current_stanzas = {i: s for i, s in enumerate(stanzas, 1) if s["stanza_sheet"] == stanza_sheet}
            ...
            if fn in system_counts:
                current_template = current_template.replace("% SYSTEM_COUNT", "system-count = "+system_counts[fn], 1)
            ...
            podnadpis = ", ".join(OrderedSet(s["section"] for s in current_stanzas.values()))
            tex_source += f"\\def\\podnadpis{{{podnadpis}}}\n"
            pocet_slok = len(current_stanzas)
            max_sloka = stanza_nos[-1]
            min_sloka = stanza_nos[0]
            tex_source += f"\\def\\minsloka{{{min_sloka}}}\n\\def\\maxsloka{{{max_sloka}}}\n"
            tex_source += f"\\def\\pretokslok{{{(pocet_slok-1)%3}}}\n"
            ...
            for s_no, s in list(current_stanzas.items())[1:]:
                ...
                stanza_lyrics = s["lyrics"].replace("--", "").replace("_", " ").replace("[:", "\n[:").split("\n")
                tex_source += f"\\sloka{{{s_no}}}{{{curr_section}}}{{%\n"
                tex_source += "\n".join(f"    \\slokaline{{{sl}}}" for sl in map(lambda x: x.strip(), stanza_lyrics) if sl)
                tex_source += "}\n"
            ...
            for shift in trs:
                ...
                for i, s in enumerate(current_stanzas.values()):
                    ...
                    if section:
                        if section == last_section:
                            section_counter += 1
                            piece = section + " " + str(section_counter)
                        else:
                            piece = section
                            section_counter = 1
                        curr_score_template = curr_score_template.replace("% SCORE_TITLE", f'piece = "{piece}"', 1)
                ...
                subprocess.run(["luatex", "--jobname="+fn_shift, "--output-directory=../pdf", tex_templates[song_type]], shell=True, capture_output=True)